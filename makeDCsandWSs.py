...
class DirectoryCreator:
    ...
    def __init__(self, args):
        ...
        if self.blind:
            ...
            if args.step == "ri" or args.step == "riess":
                ...
                if self.signalStrength != -1.0:
                    self.blindString += " --expectSignal {} ".format(
                        self.signalStrength
                    )
            if args.step == "fs":
                self.blindString = " -t -1  "
        ...
        self.datacards = (
            [self.DATA_CARD_FILENAME] if not self.allDatacard else datacardList
        )
    ...
    def ResetBlindString(self):
        if self.blind:
            ...
            if self.step == "ri" or self.step == "riess":
                self.blindString = " -t -1  --expectSignal {} ".format(
                    self.signalStrength
                )
            if self.step == "fs":
                self.blindString = " -t -1  "
    ...
    def GetCategory(self, datacard):
        return ((((datacard.replace("hzz2l2q_", "")
                  ).replace("_13TeV", "")
                  ).replace(".txt", "")
            ).replace("_xs", "")
        ).replace("13TeV", "")
    ...
    def create_workspaces(self, current_mass, datacard_class):
        ...
        border_msg(
            "Creating datacards and workspaces for mass: {}".format(current_mass)
        )
        ...
        for channel in self.channels:
            for cat in self.cats:
                ...
                datacard_class.makeCardsWorkspaces(
                    current_mass,
                    self.is_2d,
                    self.dir_name,
                    theInputs,
    ...
    def run_text2workspace(self, current_mass, current_mass_directory, datacard):
        ...
        with cd(current_mass_directory):
            ...
            if os.path.exists(datacard.replace(".txt", ".root")):
                logger.warning(
                    "Removing existing workspace: {}".format(
                        datacard.replace(".txt", ".root")
                    )
                )
            ...
            command = (
                "text2workspace.py {datacard}.txt  -m {mH} -o {datacard}.root".format(
                    datacard=datacard.replace(".txt", ""), mH=current_mass
                )
            )
    ...
    def combine_cards(self, current_mass, current_mass_directory, cwd):
        ...
        for t in self.t_values:
            ...
            for fs in ["eeqq_{}".format(t), "mumuqq_{}".format(t)]:
                RunCommand(
                    "combineCards.py {FinalState}_untagged=hzz2l2q_{FinalState}_untagged_13TeV.txt {FinalState}_b_tagged=hzz2l2q_{FinalState}_b_tagged_13TeV.txt {FinalState}_vbf_tagged=hzz2l2q_{FinalState}_vbf_tagged_13TeV.txt > hzz2l2q_{FinalState}_13TeV.txt".format(
                        FinalState=fs
                    ),
                    self.dry_run,
                )
            ...
            RunCommand(
                "combineCards.py mumuqq_{Category}=hzz2l2q_mumuqq_{Category}_13TeV.txt eeqq_{Category}=hzz2l2q_eeqq_{Category}_13TeV.txt > hzz2l2q_{Category}_13TeV_xs.txt".format(
                    Category=t
                ),
                self.dry_run,
            )
            ...
            for cat in self.cats:
                RunCommand(
                    "combineCards.py eeqq_{Category}_{Tag}=hzz2l2q_eeqq_{Category}_{Tag}_13TeV.txt mumuqq_{Category}_{Tag}=hzz2l2q_mumuqq_{Category}_{Tag}_13TeV.txt > hzz2l2q_{Category}_{Tag}_13TeV.txt".format(
                        Category=t, Tag=cat
                    ),
                    self.dry_run,
                )
            ...
            AllCardsCombination = (
                AllCardsCombination
                + " {Category}=hzz2l2q_{Category}_13TeV_xs.txt".format(Category=t)
            )
        RunCommand(
            "{stringg}\t {mH} \t {stringg}".format(stringg="*" * 11, mH=current_mass)
        )

        AllCardsCombination = AllCardsCombination + " > hzz2l2q_13TeV_xs_NoNuisance.txt"
        AllCardsWithNuisance = (AllCardsCombination.replace("-s", "  ")).replace(
            "_NoNuisance", ""
        )
    ...
    def combine_cards_allYears(self, current_mass, current_mass_directory, cwd):
        ...
        for year in ["2016", "2017", "2018"]:
            ...
            AllCardsCombination = (
                AllCardsCombination
                + " Era{year}=../../../{Run2_Cards}/HCG/{mH}/{datacard}".format(
                    mH=current_mass,
                    year=year,
                    datacard=self.DATA_CARD_FILENAME,
                    additionalString=(
                        "" if self.append_name == "" else "_" + self.append_name
                    ),
                    Run2_Cards=(self.dir_name)
                    .replace("datacards_HIG_23_001/", "")
                    .replace("run2", year),
                )
            )
        AllCardsCombination = AllCardsCombination + " > {datacard}".format(
            datacard="hzz2l2q_13TeV_xs_NoNuisance.txt"
        )
        AllCardsCombination = AllCardsCombination + " > hzz2l2q_13TeV_xs_NoNuisance.txt"
        AllCardsWithNuisance = (AllCardsCombination.replace("-s", "  ")).replace(
            "_NoNuisance", ""
    ...
    def run_combine(self, current_mass, current_mass_directory, cwd):
        ...
        for datacard in self.datacards:
            AppendNameString = CombineStrings.COMBINE_ASYMP_LIMIT.format(
                year=self.year,
                mH=current_mass,
            ...
            CombineCommonArguments = (
                " -M AsymptoticLimits -d {datacard} -m {mH}   -n .{name} ".format(
                    mH=current_mass, datacard=datacard, name=AppendNameString
                )
            )
            CombineCommonArgumentsHybrid = " -M HybridNew --LHCmode LHC-limits -d {datacard} -m {mH}   -n .{name}Hybrid ".format(
                mH=current_mass, datacard=datacard, name=AppendNameString
            )
            ...
            if self.ifCondor:
                ...
                Condor_queue = datacardList_condor[
                    datacard
                ]  # Get the condor queue from the dictionary datacardList_condor defined in the file ListOfDatacards.py
                if self.year == "run2":
                    command += self.CombineCondor.format(
                        name=AppendNameString + "_AsympLimit",
                ...
                commandHybrid += self.CombineCondor.format(
                    name=AppendNameString + "_Hybrid",
                    FitType=self.FitType,
                    JobFlavour="tomorrow",
                    Additional="",
                )
                ...
                # Change the respective directory where all cards are placed
                os.chdir(current_mass_directory)
                command = "combine  {CombineCommonArguments} | tee {name}.log".format(
                    CombineCommonArguments=CombineCommonArguments, name=AppendNameString
                )
    ...
    def run_impact_expSS(self, current_mass, current_mass_directory, cwd):
        ...
        with cd(current_mass_directory):
            ...
            for datacard in self.datacards:
                ...
                logger.debug(
                    "Opening file: {}".format(
                        "{name}*.out".format(name=AppendNameString + "_AsympLimit")
                    )
                )
                ...
                if sorted_files:
                    ...
                    logger.debug(
                        "The most recently created file is {}".format(latest_file)
                    )
                else:
                ...
                with open(latest_file, "r") as f:
                    for line in f:
                        if "Expected 50.0%: r <" in line:
                            expUL = float(
                                line.split("Expected 50.0%: r <")[1].split(" ")[1]
                            )
    ...
    def run_impact(self, current_mass, current_mass_directory, cwd):
        ...
        with cd(current_mass_directory):
            ...
            for datacard in self.datacards:
                logger.debug(
                    "===> Submitting {}/{}".format(countDatacards, len(self.datacards))
                )
                ...
                if not os.path.exists(datacard.replace(".txt", ".root")):
                    command = "text2workspace.py {datacard}.txt  -m {mH} -o {datacard}.root".format(
                        datacard=datacard.replace(".txt", ""), mH=current_mass
                    )
                    RunCommand(command, self.dry_run)
                ...
                if self.substep == 1:
                    # STEP - 1
                    command = "combineTool.py -M Impacts -d {datacard}  -m {mH} -n .{name}  --robustFit 1 --doInitialFit ".format(
                ...
                if self.substep == 2:
                    # STEP - 2
                    command = "combineTool.py -M Impacts -d {datacard}  -m {mH}  -n .{name} --robustFit 1 --doFits ".format(
                        datacard=datacard.replace(".txt", ".root"),
                        mH=current_mass,
                        name=AppendOutName,
                    )
                ...
                if self.substep == 3:
                    # STEP - 3
                    command = "combineTool.py -M Impacts -d {datacard} -m {mH}  -n .{name}   --robustFit 1   --output impacts_{name}.json".format(
                        datacard=datacard.replace(".txt", ".root"),
                        mH=current_mass,
                        name=AppendOutName,
                    )
                    ...
                    command = "plotImpacts.py -i impacts_{name}.json -o {pathh}/impacts/impacts_{name} ".format(
                        pathh="../../figs", name=AppendOutName
                    )  # --blind
                    RunCommand(command, self.dry_run)
                countDatacards += 1

    def run_LHS(
        self, current_mass, current_mass_directory, cwd
    ):  # Commands taken From Ankita
        ...
        for datacard in self.datacards:
            logger.debug("Datacard: {}".format(datacard))
            # if datacard.root file exists, then skip this step
            if not os.path.exists(datacard.replace(".txt", ".root")):
                command = "text2workspace.py {datacard}.txt  -m {mH} -o {datacard}.root".format(
                    datacard=datacard.replace(".txt", ""), mH=current_mass
                )
                logger.debug("workspace creation command: {}".format(command))
                RunCommand(command, self.dry_run)
                logger.debug("workspace created for datacard: {}".format(datacard))

            OutNameAppend = CombineStrings.COMBINE_FITDIAGNOSTIC.format(
                year=self.year,
                mH=current_mass,
                blind="blind" if self.blind else "",
                Category=self.GetCategory(datacard),
            ...
            if not self.blind:
                logger.debug("Analysis is unblinded")
                pass
            else:
                logger.debug("Analysis is blinded")
                pointsToScan = 200
                rRange = "-3,3"  # for ExpectedSignal = 1
                CommonArguments = " --algo grid --points {pointsToScan}".format(
                    pointsToScan=pointsToScan
                )
                name = (
                    "nominal.with_syst_" + "points" + str(pointsToScan)
                )  # String append to the name of floting pars
                name2 = (
                    "bestfit.with_syst_" + "points" + str(pointsToScan)
                )  # String append to the name of AllConstrainedNuisances
                name3 = (
                    "scan.with_syst.statonly_correct_" + "points" + str(pointsToScan)
                )  # String append to the name of AllConstrainedNuisances
                outPDFName = (
                    OutNameAppend + "_points" + str(pointsToScan)
                )  # Output PDF File

                CustomString = CombineStrings.COMBINE_IMPACT.format(
                    year=self.year,
                    mH=current_mass,
                    blind="blind" if self.blind else "",
                    Category=self.GetCategory(datacard),
                ...
                CondorCommandPatch = " -v -1 " + self.CombineCondor.format(
                    name=CustomString + "_LHS",
                    FitType=self.FitType,
                    JobFlavour="tomorrow",
                    Additional="\\nRequestCpus=4\\nrequest_memory = 10000",
                )
                ...
                if self.substep == 1:
                    # STEP - 1
                    command = " -M MultiDimFit --algo grid --points {pointsToScan} --rMin -3 --rMax 3 -d {datacard} -m {mH} -n .{name} --saveWorkspace {blindString}  ".format(
                        pointsToScan=pointsToScan,
                        datacard=datacard.replace(".txt", ".root"),
                        mH=current_mass,
                        blindString=self.blindString,
                        name=name,
                    )
                    # while running on condor, switch the verbosity level to -1 (very quite)
                    logger.debug("Condor: {}".format(self.ifCondor))
                    if self.ifCondor:
                        command = (
                            "combineTool.py " + command + CondorCommandPatch + "Step1"
                        )
                    else:
                ...
                if self.substep == 2:
                    # STEP - 2
                    command = " -M MultiDimFit --algo none --rMin -3 --rMax 3 -d {datacard} -m {mH} -n .{name} --saveWorkspace {blindString} ".format(
                        datacard=datacard.replace(".txt", ".root"),
                        mH=current_mass,
                        blindString=self.blindString,
                        name=name2,
                    ...
                    if self.ifCondor:
                        command = (
                            "combineTool.py " + command + CondorCommandPatch + "Step2"
                        )
                    else:
                        command = "combine " + command
                    ...
                    # STEP - 3
                    command = " -M MultiDimFit higgsCombine.{name}.MultiDimFit.mH{mH}.root -m {mH} --freezeParameters allConstrainedNuisances -n .{name3} --rMin -3 --rMax 3 --snapshotName MultiDimFit {blindString} --algo grid --points {pointsToScan} ".format(
                        name=name,
                        name3=name3,
                        mH=current_mass,
                        blindString=self.blindString,
                    ...
                    if self.ifCondor:
                        command = (
                            "combineTool.py " + command + CondorCommandPatch + "Step3"
                        )
                    else:
                        command = "combine " + command
                ...
                if self.substep == 3:
                    # STEP - 4
                    command = 'plot1DScan.py higgsCombine.{name}.MultiDimFit.mH{mH}.root --main-label "With systematics" --main-color 1 --others higgsCombine.{name3}.MultiDimFit.mH{mH}.root:"Stat-only":2 -o {outPDFName} --breakdown syst,stat'.format(
                        datacard=datacard.replace(".txt", ".root"),
                        mH=current_mass,
                        pointsToScan=pointsToScan,
                        name=name,
                        name3=name3,
                        outPDFName="../../figs/" + outPDFName,
                    )
                    RunCommand(command + " | tee LHS_plot.log", self.dry_run)
                ...
                if self.substep == 21:
                    ...
                    command = "combineTool.py -M MultiDimFit --algo grid --points {pointsToScan} --rMin 0 --rMax 3 {datacard} --alignEdges 1 --floatOtherPOIs=1  -n .likelihoodscan --saveWorkspace  -m {mH} ".format(
                        pointsToScan=pointsToScan,
                        datacard=datacard.replace(".txt", ".root"),
                        mH=current_mass,
                    )
                    if self.ifCondor:
                        command = (
                            "combineTool.py " + command + CondorCommandPatch + "Step21"
                        )
                    else:
                        command = "combine " + command
                    RunCommand(command + " ", self.dry_run)
                ...
                if self.substep == 23:
                    ...
                    command = "combine -M MultiDimFit higgsCombine.likelihoodscan.MultiDimFit.mH{mH}.root -n .likelihoodscan.freezeAll -m {mH} --rMin 0 --rMax 3  --algo grid --points {pointsToScan} --freezeParameters allConstrainedNuisances --snapshotName MultiDimFit --alignEdges 1 ".format(
                        pointsToScan=pointsToScan, mH=current_mass
                    )
                    if self.ifCondor:
                        command = (
                            "combineTool.py " + command + CondorCommandPatch + "Step23"
                        )
                    else:
                        command = "combine " + command
                    RunCommand(command + " ", self.dry_run)
                ...
                if self.substep == 24:
                    ...
                    command = 'plot1DScan.py higgsCombine.likelihoodscan.MultiDimFit.mH125.root --y-cut 50 --y-max 50 --breakdown syst,stat --others "higgsCombine.likelihoodscan.freezeAll.MultiDimFit.mH{mH}.root:Stat only:2"'.format(
                        mH=current_mass
                    )
                    RunCommand(command + " ", self.dry_run)
    ...
    def run_2DLHS(self, current_mass, current_mass_directory, cwd):
        ...
        logger.info("Getting the workspace for the datacard")
        ...
        command = 'text2workspace.py {datacard}.txt -m {mH} -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO "map=.*/ggH:r_ggH[1,-2,2]" --PO "map=.*/VBF:r_VBF[1,-2,2]" -o {datacard}.root'.format(
            datacard=self.DATA_CARD_FILENAME.replace(".txt", ""), mH=current_mass
        )
        # RunCommand(command, self.dry_run)
        ...
        OutNameAppend = CombineStrings.COMBINE_FITDIAGNOSTIC.format(
            ...
            mH=current_mass,
            blind="blind" if self.blind else "",
            Category=self.GetCategory(self.DATA_CARD_FILENAME),
            bOnlyOrSB=self.FitType,
            Additional="\\nRequestCpus=4\\nrequest_memory = 10000",
        )
        ...
        if not self.blind:
            ...
            name = (
                "scan2D.with_syst_" + "points" + str(pointsToScan)
            )  # String append to the name of floting pars
            name2 = (
                "scan.with_syst.statonly_correct_" + "points" + str(pointsToScan)
            )  # String append to the name of AllConstrainedNuisances
            ...
            # -P r_ggH -P r_VBF --setParameterRanges r_ggH=-2,2:r_VBF=-2,2
            ...
            command = "combine -M MultiDimFit {datacard} -m {mH} --freezeParameters MH -n .{name}_fastScan_{pointsToScan} --cminDefaultMinimizerStrategy 0 --robustFit 1 --fastScan  ".format(
                datacard=self.DATA_CARD_FILENAME.replace(".txt", ".root"),
                mH=current_mass,
                pointsToScan=pointsToScan,
                name=name,
            )
            ...
            # 2D likelihood scan
            # command = "combine -M MultiDimFit {datacard} -m {mH} --freezeParameters MH -n .{name}_ggHVBF  --cminDefaultMinimizerStrategy 0   ".format(datacard=self.DATA_CARD_FILENAME.replace(".txt",".root"), mH=current_mass, pointsToScan = pointsToScan, name = name)
            ...
            command += Stat
            command += self.blindString
            RunCommand(command + " | tee LHS2D_Float.log", self.dry_run)
    ...
    def run_correlation(self, current_mass, current_mass_directory, cwd):
        ...
        command = (
            "text2workspace.py "
            + self.DATA_CARD_FILENAME
            + " -m "
            + str(current_mass)
            + " -o "
            + self.DATA_CARD_FILENAME.replace(".txt", ".root")
        )
        RunCommand(command, self.dry_run)
        ...
        if not self.blind:
            ...
            command = "combine -M MultiDimFit {datacard} -m {mH} --freezeParameters MH -n .correlation --cminDefaultMinimizerStrategy 0  --robustHesse 1 --robustHesseSave 1 --saveFitResult  -t -1 --expectSignal  {ExpectedSignal} ".format(
                datacard=self.DATA_CARD_FILENAME.replace(".txt", ".root"),
                mH=current_mass,
    ...
    def fastScan(self, current_mass, current_mass_directory, cwd):
        ...
        for datacard in self.datacards:
            ...
            if self.blind:
                command += " -d higgsCombine.{name}.GenerateOnly.mH{mH}.123456.root:toys/toy_asimov -o ../../figs/fastScan_{name} ".format(
                    mH=current_mass, name=OutNameAppend + "_" + self.FitType
                )
    ...
    def run_parallel(self, current_mass):
        border_msg(
            "Running step {step} for mass {current_mass}, year: {year}".format(
                step=self.step, current_mass=current_mass, year=self.year
            )
        )
        actions = {
        ...
        logger.debug(
            "Running step {step} for mass {current_mass} in directory {current_mass_directory}".format(
                step=self.step,
                current_mass=current_mass,
                current_mass_directory=current_mass_directory,
            )
        )
        logger.debug(
            "step: {step}, current_mass: {current_mass}, current_mass_directory: {current_mass_directory}, cwd: {cwd}".format(
                step=self.step,
                current_mass=current_mass,
                current_mass_directory=current_mass_directory,
                cwd=cwd,
        ...
        if self.step.lower() == "all":
            ...
            if (
                action is not None
            ):  # FIXME: Need to add condition that when the year is `run2` then for combining cards we need to use `run2` function instead of `cc`
    ...
    def run_step_dc(self):
        ...
        if self.ifParallel:
            ...
            try:
                ...
                arguments = [
                    (current_mass, datacard_class)
                    for current_mass in range(
                        self.start_mass, self.end_val, self.step_sizes
                    )
                ]
            ...
            for current_mass in range(self.start_mass, self.end_val, self.step_sizes):
                ...
                RunCommand(
                    "mv git_diff.patch {}/".format(
                        self.dir_name + "/HCG/" + str(current_mass)
                    ),
                    self.dry_run,
                )
    ...
    def run_step_plot(self):
        ...
        for datacard in self.datacards:
            SearchString4Datacard = CombineStrings.COMBINE_ASYMP_LIMIT.format(
                year=self.year,
                mH="REPLACEMASS",
                blind="blind" if self.blind else "",
                Category=self.GetCategory(datacard),
            ...
            command = "combineTool.py -M CollectLimits {pathh}/HCG/*/higgsCombine.{name}.AsymptoticLimits.mH*.root --use-dirs -o {oPath}/limits_{name2}.json".format(
                pathh=self.dir_name,
                name=SearchString4Datacard.replace("REPLACEMASS", "*"),
                name2=SearchString4Datacard.replace("mHREPLACEMASS", "Summary"),
                oPath=os.path.join(self.dir_name, "figs"),
            )
            RunCommand(command, self.dry_run)

            # Plot the limits
            command = (
                "python plotLimitExpObs_2D.py {}  {}  {}  {} {} {} {} {} {}".format(
                    self.start_mass,
                    self.end_val,
                    self.step_sizes,
                    self.year,
    ...
    def Run(self):
        ...
        if (self.step).lower() in (
            "dc"
        ):  # or (self.step).lower() == 'all': # Fixme: all year is not working for 'dc'
            self.run_step_dc()
            ...
            if not self.ifParallel:
                ...
                try:
                    pool.map(
                        partial(run_parallel_instance, self),
                        range(self.start_mass, self.end_val, self.step_sizes),
                    )
                finally:
                    pool.close()
                    pool.join()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run combine for high mass Higgs search analysis"
    )
    ...
    # General Settings
    general_settings.add_argument(
        "-i", "--input", dest="input_dir", type=str, default="", help="inputs directory"
    )
    general_settings.add_argument(
        "-d", "--is2D", dest="is_2d", type=int, default=1, help="is2D (default:1)"
    )
    general_settings.add_argument(
        "-a",
        "--append",
        ...
        type=str,
        default="",
        help="append name for cards dir",
    )
    general_settings.add_argument(
        "--dry-run",
        action="store_true",
        help="Don't actually run the command, just print it.",
    )
    general_settings.add_argument(
        "-p", "--parallel", action="store_true", help="Run jobs parallelly"
    )
    ...
    mass_settings.add_argument(
        ...
        type=int,
        default=50,
        help="MassStepVal (default:1)",
    )

    # Year and Condor Settings
    year_condor_settings.add_argument(
        "-y",
        "--year",
        dest="year",
        type=str,
        default="2016",
        help="year to run or run for all three year. Options: 2016, 2017, 2018,all (=16, 17 and 18), allc (=all + run2), run2",
    )
    year_condor_settings.add_argument(
        "-c",
        "--ifCondor",
        ...
        dest="ifCondor",
        default=False,
        help="if you want to run combine command for all mass points parallel using condor make it 1",
    )
    ...
    fit_settings.add_argument(
        ...
        dest="allDatacard",
        default=False,
        help="If we need limit values or impact plot for each datacards, stored in file ListOfDatacards.py",
    )
    fit_settings.add_argument(
        "-f",
        "--fracVBF",
        ...
        type=float,
        default=-1,
        help="fracVBF, -1 means float this frac. (default:-1)",
    )
    fit_settings.add_argument(
        "-b",
        "--blind",
    ...
    fit_settings.add_argument(
        ...
        type=str,
        default="--run blind",
        help="howToBlind: --run blind, -t -1 --expectSignal 0, -t -1 --expectSignal 1",
    )
    fit_settings.add_argument(
        "-signalStrength",
        "--signalStrength",
        ...
        type=float,
        default=0.0,
        help="signal strength for the fit",
    )
    fit_settings.add_argument(
        "-freezeParameters",
        "--freezeParameters",
        dest="freezeParameters",
        type=str,
        default="",
        help="freeze parameters for the fit. If want to freeze all then give argumen `allConstrainedNuisances`",
    )
    fit_settings.add_argument(
        "-setParameterRanges",
        "--setParameterRanges",
        dest="setParameterRanges",
        type=str,
        default="",
        help="set parameters range for the fit. Its format should be `r=-1,3:BTAG_resolved=-5,5:BTAG_merged=-5,5`",
    )
    fit_settings.add_argument(
        "-AdditionalFitOptions",
        "--AdditionalFitOptions",
        dest="AdditionalFitOptions",
        type=str,
        default="",
        help="AdditionalFitOptions for the fit. Its format should be `--cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance 0.01 --setRobustFitTolerance 0.01`",
    )

    # Logging Settings
    logging_settings.add_argument(
        "--log-level",
    ...
    logging_settings.add_argument(
        ...
        type=int,
        default=0,
        help="combineVerbose for the fit. Its format should be `--verbose 0`",
    )
    ...
    advanced_settings.add_argument(
        ...
        type=str,
        default="",
        help="If this option is given, then it will append date string to the output file name. This is helpful if I want to use the combine output from different date",
    )
    advanced_settings.add_argument(
        "-tag",
        "--tag",
        ...
        type=str,
        default="",
        help="This option add additional string in the combine output root file as well as condor/combine log file",
    )
    advanced_settings.add_argument(
        "-sanityCheck",
        "--sanity-check",
        action="store_true",
        dest="SanityCheckPlotUsingWorkspaces",
        default=False,
        help="If this option is given, then it will plot the sanity check plots using workspaces",
    )

    # Step Control
    step_control.add_argument(
        "-s",
        "--step",
        dest="step",
        type=str,
        default="dc",
        help="Which step to run: dc (DataCardCreation), cc (CombineCards), ws (Get workspaces) rc (RunCombine), fd (Fit Diagnostics) ri (run Impact), fs (fastScan), rll (run loglikelihood with and without syst) , corr (Correlation), plot or all",
        choices=[
            "dc",
            "cc",
            "ws",
    ...
    step_control.add_argument(
        "-ss", "--substep", dest="substep", type=int, default=11, help="sub-step help"
    )
    ...
    for year in years:
        if str(year).lower() == "run2":
            RunCommand(
                "ulimit -s unlimited", args.dry_run
            )  # this is needed for run2 combination otherwise it will give segmentation fault. Not sure why.
