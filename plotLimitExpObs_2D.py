...
for current_mass in range(int(start_mass), int(end_val), int(step_sizes)):
        ...
        f = TFile("./datacards_HIG_23_001/cards_"+str(year)+"/HCG/"+str(m)+"/higgsCombine"+common_strings_pars.COMBINE_ASYMP_LIMIT.format(year = year, mH = current_mass)+".AsymptoticLimits.mH"+str(m)+".root","READ")
        t = f.Get("limit")
