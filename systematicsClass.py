...
class systematicsClass:
    ...
    def __init__(self, theMass, theForXSxBR, theInputs, year, DEBUG):
        ...
        self.qqVV_scaleSys = 1.0 + 0.01 * math.sqrt((self.mH - 20.0) / 13.0)
        self.qqVV_pdfSys = 1.0 + 0.0035 * math.sqrt(self.mH - 30.0)
        self.DEBUG = DEBUG
    ...
    def Write_pdf_qqbar(self, theFile, theInputs):
        ...
        if not self.isForXSxBR:
            systLine = {"ggH": "- "}
            systLine["qqH"] = (
                "1.021 "  # "{0:.4f} ".format(1. + (self.CSpdfErrPlus_vbf-self.CSpdfErrMinus_vbf)/2.)
            )
            systLine["zjets"] = "- "
            systLine["ttbar"] = "- "
            systLine["vz"] = "1.03 "
    ...
    def Write_QCDscale_vz(self, theFile, theInputs):
        ...
        systLine = {
            "ggH": "- "
        }  # "{0:.4f} ".format(1. + (self.CSscaleErrPlus_gg-self.CSscaleErrMinus_gg)/2.)}
        systLine["qqH"] = "- "
        systLine["zjets"] = "- "
        systLine["ttbar"] = "- "
        systLine["vz"] = "1.032 "
    ...
    def Write_QCDscale_qqH(self, theFile, theInputs):
        ...
        systLine["qqH"] = (
            "1.004/0.997 "  # "{0:.4f} ".format(1. + (self.CSscaleErrPlus_vbf-self.CSscaleErrMinus_vbf)/2.)
        )
        systLine["zjets"] = "- "
        systLine["ttbar"] = "- "
        systLine["vz"] = "- "
    ...
    def Write_Ak4_Split_JEC(self, theFile, theinputs):
        ...
        for source in self.SplitSourceYears:
            ...
            systLine["zjets"] = "- "
            systLine["ttbar"] = "{} ".format(
                theinputs["{}_year".format(source)]["ttbar"]
            )
    ...
    def Write_Ak4_deepjet_Split_JEC(self, theFile, theinputs):
        ...
        for flav in ["bc", "light"]:
            ...
            systLine = {
                "ggH": "{} ".format(theinputs["deepjetsf{}".format(flav)]["ggH"])
            }
            systLine["qqH"] = "{} ".format(theinputs["deepjetsf{}".format(flav)]["qqH"])
            systLine["zjets"] = "- "
            systLine["ttbar"] = "{} ".format(
                theinputs["deepjetsf{}".format(flav)]["ttbar"]
            )
            ...
            systLine = {
                "ggH": "{} ".format(
                    theinputs["deepjetsfcorrelated{}".format(flav)]["ggH"]
                )
            }
            systLine["qqH"] = "{} ".format(
                theinputs["deepjetsfcorrelated{}".format(flav)]["qqH"]
            )
            systLine["zjets"] = "- "
            systLine["ttbar"] = "{} ".format(
                theinputs["deepjetsfcorrelated{}".format(flav)]["ttbar"]
            )
            systLine["vz"] = "{} ".format(
                theinputs["deepjetsfcorrelated{}".format(flav)]["vz"]
            )
    ...
    def WriteSystematics(self, theFile, theInputs, rates, Nemu):
        ...
        if theInputs["useLumiUnc"]:
            ...
            if str(self.year) != "2016":
                self.Build_lumi_Corr17_18(theFile, theInputs)
